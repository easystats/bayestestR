...
for(i in 4:nrow(data)){
  ...
  param <- bayestestR::describe_posterior(model, test=c("pd", "ROPE", "p_MAP"))[2,]
...
p_significance <- params %>%
  select(Evidence, pd, ROPE_Percentage, BF_null) %>%
  tidyr::pivot_longer(cols=-Evidence, names_to = "Index", values_to = "Value") %>%
  ggplot(aes(x=Evidence, y=Value, color=Index, group=1)) +
  geom_line() +
  scale_color_manual(values = c("pd"="red", "ROPE_Percentage"="orange", "BF_null"="green"), guide=FALSE) +
