...
test_that("rope", {
  testthat::expect_equal(as_numeric_rope(rope(rnorm_perfect(1000, 0, 1))), 8.88, tolerance = 0.01)
  testthat::expect_equal(equivalence_test(rnorm_perfect(1000, 0, 1)), "undecided")

  testthat::expect_equal(as_numeric_rope(rope(rnorm_perfect(1000, 2, 0.01))), 0, tolerance = 0.01)
  testthat::expect_equal(equivalence_test(rnorm_perfect(1000, 2, 0.01)), "rejected")

  testthat::expect_equal(as_numeric_rope(rope(rnorm_perfect(1000, 0, 0.001))), 100, tolerance = 0.01)
  testthat::expect_equal(equivalence_test(rnorm_perfect(1000, 0, 0.001)), "accepted")

  testthat::expect_equal(as_numeric_rope(rope(rnorm_perfect(1000, 1, 1), CI = c(50, 99))$CI_99), 4.94, tolerance = 0.01)
  ...
  testthat::expect_equal(as_numeric_rope(rope(rnorm_perfect(1000, 0, 1), bounds = c(-0.1, 0.1))), 8.88, tolerance = 0.01)
})
