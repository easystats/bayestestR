...
[`library`](https://rdrr.io/r/base/library.html)`(`[`mediation`](https://imai.princeton.edu/projects/mechanisms.html)`)`\
...
\
`# load sample data`\
[`data`](https://rdrr.io/r/utils/data.html)`(``jobs``)`\
\
[`set.seed`](https://rdrr.io/r/base/Random.html)`(``123``)`\
`# linear models, for mediation analysis`\
`b1`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``job_seek`` ``~`` ``treat`` ``+`` ``econ_hard`` ``+`` ``sex`` ``+`` ``age``, data ``=`` ``jobs``)`\
`b2`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``depress2`` ``~`` ``treat`` ``+`` ``job_seek`` ``+`` ``econ_hard`` ``+`` ``sex`` ``+`` ``age``, data ``=`` ``jobs``)`\
\
`# mediation analysis, for comparison with brms`\
`m1`` ``<-`` `[`mediate`](https://rdrr.io/pkg/mediation/man/mediate.html)`(``b1``, ``b2``, sims ``=`` ``1000``, treat ``=`` ``"treat"``, mediator ``=`` ``"job_seek"``)`

\
`# Fit Bayesian mediation model in brms`\
`f1`` ``<-`` `[`bf`](https://paulbuerkner.com/brms/reference/brmsformula.html)`(``job_seek`` ``~`` ``treat`` ``+`` ``econ_hard`` ``+`` ``sex`` ``+`` ``age``)`\
`f2`` ``<-`` `[`bf`](https://paulbuerkner.com/brms/reference/brmsformula.html)`(``depress2`` ``~`` ``treat`` ``+`` ``job_seek`` ``+`` ``econ_hard`` ``+`` ``sex`` ``+`` ``age``)`\
`m2`` ``<-`` `[`brm`](https://paulbuerkner.com/brms/reference/brm.html)`(``f1`` ``+`` ``f2`` ``+`` `[`set_rescor`](https://paulbuerkner.com/brms/reference/brmsformula-helpers.html)`(``FALSE``)``, data ``=`` ``jobs``, refresh ``=`` ``0``)`

\
`# Fit Bayesian mediation model in rstanarm`\
`m3`` ``<-`` `[`stan_mvmer`](https://mc-stan.org/rstanarm/reference/stan_mvmer.html)`(`\
...
\
`# for brms`\
[`mediation`](https://easystats.github.io/bayestestR/reference/mediation.md)`(``m2``)`\
...
\
`# for rstanarm`\
[`mediation`](https://easystats.github.io/bayestestR/reference/mediation.md)`(``m3``)`\
`#> # Causal Mediation Analysis for Stan Model`\
`#> `\
`#>   Treatment: treat`\
`#>   Mediator : job_seek`\
...
`#> Simulations: 1000`\
\
[`mediation`](https://easystats.github.io/bayestestR/reference/mediation.md)`(``m2``, ci ``=`` ``0.95``)`\
`#> # Causal Mediation Analysis for Stan Model`\
`#> `\
`#>   Treatment: treat`\
`#>   Mediator : job_seek`\
...
`#> Proportion mediated: 28.14% [-181.46%, 237.75%]`\
\
[`mediation`](https://easystats.github.io/bayestestR/reference/mediation.md)`(``m3``, ci ``=`` ``0.95``)`\
`#> # Causal Mediation Analysis for Stan Model`\
`#> `\
`#>   Treatment: treat`\
`#>   Mediator : job_seek`\
...
\
`m`` ``<-`` `[`mediation`](https://easystats.github.io/bayestestR/reference/mediation.md)`(``m2``, centrality ``=`` ``"mean"``, ci ``=`` ``0.95``)`\
[`print`](https://rdrr.io/r/base/print.html)`(``m``, digits ``=`` ``4``)`\
`#> # Causal Mediation Analysis for Stan Model`\
`#> `\
`#>   Treatment: treat`\
`#>   Mediator : job_seek`\
...
`             total_age := c4 + (a4*b)`\
`         "`\
`m4`` ``<-`` `[`sem`](https://rdrr.io/pkg/lavaan/man/sem.html)`(``model``, data ``=`` ``jobs``)`\
[`summary`](https://rdrr.io/r/base/summary.html)`(``m4``)`\
