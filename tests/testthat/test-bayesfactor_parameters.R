if (require("rstanarm") &&
  ...
  if (.runThisTest) {
    test_that("bayesfactor_parameters BRMS", {
      ...
      bfsd1 <- bayesfactor_parameters(brms_mixed_6, brms_mixed_6_p, effects = "fixed", verbose = FALSE)

      set.seed(222)
      bfsd2 <- bayesfactor_parameters(brms_mixed_6, effects = "fixed", verbose = FALSE)
