...
#' \cr
#' In most cases, it seems that the \emph{pd} chas a direct correspondance with the frequentist one-sided \emph{p}-value through the formula \ifelse{html}{\out{p<sub>one sided</sub> = 1-p<sub>d</sub>/100}}{\eqn{p_{one sided} = 1-\frac{p_{d}}{100}}} and to the two-sided p-value (the most commonly reported one) through the formula \ifelse{html}{\out{p<sub>one sided</sub> = 2*(1-p<sub>d</sub>/100)}}{\eqn{p_{one sided} = 2*(1-\frac{p_{d}}{100})}}. Thus, a two-sided p-value of respectively \code{.1}, \code{.05}, \code{.01} and \code{.001} would correspond approximately to a \code{pd} of \code{95\%}, \code{97.5\%}, \code{99.5\%} and \code{99.95\%}.
#'
#' \cr \cr
