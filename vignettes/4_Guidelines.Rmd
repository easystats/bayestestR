...
- **Probability of Direction (*p*d)**: In most cases, it seems that the *pd* corresponds to the frequentist one-sided *p*-value through the formula `p-value = (1-pd/100)`. Thus, a ***p*d** of `95%`, `97.5%` `99.5%` and `99.95%` corresponds approximately to a two-sided ***p*-value** of respectively `.1`, `.05`, `.01` and `.001`. Thus, for convience, we recommend using the following reference values:
