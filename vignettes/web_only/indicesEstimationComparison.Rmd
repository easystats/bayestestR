...
```{r, message=FALSE, warning=FALSE}
df %>%
  data_select(c("sample_size", "error", "true_effect", "outcome_type", "Coefficient", "Median", "Mean", "MAP")) %>%
  reshape_longer(
  ...
  data_select(c("Parameter", "Coefficient", "p")) %>%
  data_filter(filter = grep("^outcome_type(.*):value$", x = .$Parameter)) %>%
