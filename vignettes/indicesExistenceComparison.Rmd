...
```{r, message=FALSE, warning=FALSE, fig.height=20, fig.width=16}
df %>%
  ...
  mutate(true_effect = as.factor(true_effect),
         index = factor(index, levels=c("p_value", "p_direction", "p_MAP", "p_ROPE", "ROPE_90", "ROPE_95", "ROPE_full"))) %>%
  ...
  scale_fill_manual(values = c("p_value"="#607D8B", "p_MAP" = "#4CAF50", "p_direction" = "#2196F3",
                               "ROPE_90" = "#FFC107", "ROPE_95" = "#FF9800", "ROPE_full" = "#FF5722", "p_ROPE"="#E91E63"), guide=FALSE) +
  ylab("Index Value\n") +
  xlab("\nNoise")
```
...
df %>%
  ...
  mutate(true_effect = as.factor(true_effect),
         index = factor(index, levels=c("p_direction", "p_MAP", "p_ROPE", "ROPE_90", "ROPE_95", "ROPE_full"))) %>%
...
rbind(
  rbind(
    ...
    get_data(predictor="p_ROPE", outcome="sig_001", lbound=0, ubound=100),
  ...
  rbind(
    ...
    get_data(predictor="p_ROPE", outcome="sig_01", lbound=0, ubound=100),
  ...
  rbind(
    ...
    get_data(predictor="p_ROPE", outcome="sig_05", lbound=0, ubound=100),
...
rbind(
  ...
  geom_ribbon(aes(ymin=CI_lower, ymax=CI_upper), alpha=0.1) +
  geom_line(aes(color=index), size=1) +
  facet_wrap(~ index * level * outcome_type, scales = "free", nrow=5) +
  theme_classic() +
  theme(strip.background = element_blank(),
        strip.text = element_text(face="bold")) +
  scale_color_manual(values = c("p_value"="#607D8B", "p_MAP" = "#4CAF50", "p_direction" = "#2196F3",
