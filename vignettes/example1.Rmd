...
format_percent <- function(x, digits = 0, ...) {
  paste0(insight::format_value(x * 100, digits = digits, ...), "%")
}
```
...
framework) - see discussion on choice of credibility level [here](https://easystats.github.io/bayestestR/articles/credible_interval.html).
...
```{r echo=FALSE, , comment=NA, results='hide'}
model <- stan_glm(weight ~ feed, data = data, chains = 10, iter = 5000, warmup = 1000)
```
...
knitr::include_graphics("../man/figures/profsanders.jpg")
```
