...
We can also examine the existence of an effect by examining under which of two models is the observed data more probable: a model with the effect of interest, and a null model without the effect of interest. Such a comparison would produce a [Bayes factor](https://easystats.github.io/bayestestR/articles/bayes_factors.html). To test the effect of feed type, we can use `bayesfactor_savagedickey()` to compute the a Savage-Dickey Bayes factor:

```{r message=FALSE, warning=FALSE, eval=TRUE}
bayesfactor_savagedickey(model)
```

We got a *BF*<sub>10</sub> of ~1.5, indicating that the data is 1.5 times less probable under the null compared to the alternative hypothesis, which, though favoring the null, is considered only [weak evidence against the null](https://easystats.github.io/report/articles/interpret_metrics.html#bayes-factor-bf). This might be due to the fact we used `rstanarm`'s default priors that are very weakly-informative - unlike ROPE percentage and probability of direction, Bayes factors are sensitive to and influenced by the model / parameter priors. 
