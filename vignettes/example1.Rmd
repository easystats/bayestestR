...
Let's fit a **Bayesian version** of the model by using the `stan_glm` function in the `rstanarm` package:
...
Now that we've understood where these values come from, let's look at them. We will start by visualizing the distribution of our parameter of interest, the effect of `Petal.Length`.
...
Well, this is **very close to the mean** (and identical when rounding the values). Maybe we could take the **mode**, the *peak* of the posterior distribution? In the Bayesian framework, this value is called the **Maximum A Posteriori (MAP)**. Let's see:
...
Let's take interest in the **weight of chickens**, depending on 2 **feed types**. We will start by selecting, from the `chickwts` dataset (also available in base R), the 2 feed types that are of interest for us (*we do have peculiar interests*): **Meat meals** and **Sunflowers**.
...
This represents the **posterior distribution of the difference between meat meals and Sunflowers**. Seems that the difference is rather **positive** (the values seems concentrated on the right side of 0)... Eating Sunflowers makes you more fat (*at least, if you're a chicken*). **By how much?** Let us compute the **median** and the **CI**:
