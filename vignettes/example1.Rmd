...
We can also examine the existence of an effect by comparing the relative likelihood of two models: a model with the effect, and a null model without the effect. Such a comparison would produce a [Bayes factor](https://easystats.github.io/bayestestR/articles/bayes_factors.html). To test the effect of feed type, we can use the Savage-Dickey density ratio, by comparing the probability of the null effect under the prior distribution to the probability of the null effect under the posterior distribution. For that, can pass our model to `bayesfactor_savagedickey`:

```{r message=FALSE, warning=FALSE, eval=TRUE}
bayesfactor_savagedickey(model)
```

We got a BF of ~1.5, indicating that the data is 1.5 times less probable under the null compared to the alternative hypothesis, which, though favoring the null, is considered only [weak evidence against the null](https://easystats.github.io/report/articles/interpret_metrics.html#bayes-factor-bf). This might be due to the fact we used the default weakly-informative priors when fitting the model - unlike ROPE percentage and probability of direction, Bayes factors are more strongly influenced by the use of priors. 
