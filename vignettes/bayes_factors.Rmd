...
The adoption of the Bayesian framework for applied statistics, especially in social or psychological sciences, seems to be developing in two distinct directions. One of the key topic marking their separation is their opinion about the **Bayes factor**. In short, some authors (e.g., the "Amsterdam school", led by [Wagenmakers](https://www.bayesianspectacles.org/)) advocate its use and emphasise its qualities as a statistical index, while others point its limits and prefer the precise description of the posterior distribution (using CIs, ROPEs, etc.). 

`bayestestR` does not take a side in this debate, rather offering tools to help you in whatever analysis you want to achieve. It strongly supports the notion of an *informed choice*: **discover the methods, try them, understand them, learn about them, and decide**.

Thus, here's an introduction to Bayes factors :)
...
This is done by by comparing the density (likelihood) of the null value under the prior between the prior and posterior distributions (see @wagenmakers2010bayesian).

Let's use the Student's Sleep data, and try and answer the question: *given the observed data, is it more or less likely that the drug (the variable `group`) has no effect on the numbers of hours of extra sleep (variable `extra`?*

```{r sleep_boxplot, message=FALSE, warning=FALSE}
library(ggplot2)
library(dplyr)
...
```{r prior_n_post, message=FALSE, warning=FALSE}
...
We can not plot both the prior:

```{r prior_n_post_plot, echo=FALSE, message=FALSE, warning=FALSE}
ggplot() +
  geom_density(data=data.frame(prior), aes(x = prior, color = "Prior")) +
  geom_density(data=data.frame(posterior), aes(x = posterior, color = "Posterior")) +
...
This BF indicates the **likelihood of an effect of 0 (null effect) is `r round(test_group2$BFsd[1],2)` less likely under posterior**. In other words, the observed data has 1/`r round(test_group2$BFsd[1],2)` = `r 1/round(test_group2$BFsd[1],2)` more chances to happen under the null hypothesis. Although the centre of distribution has shifted, it is still quite dense around the null.
...
ggplot() +
  geom_density(data=data.frame(prior = prior[prior > 0]), aes(x = prior, color = "Prior")) +
  geom_density(data=data.frame(posterior = posterior[posterior > 0]), aes(x = posterior, color = "Posterior")) +
  theme_classic()
...
As we can see, given that we have an *a priori* assumption about the direction of the effect (*that the effect is positive*), the presence of an effect is `r round(test_group2_right$BFsd[1],2)` times more likely than the null hypothesis. This indicates that, given the observed data, the posterior mass has shifted away from the null value, giving some evidence against the null (note that a Bayes factor of `r round(test_group2_right$BFsd[1],2)` is still considered quite week evidence).
...
"Species * Petal.Length", "1"), log.BF = c(102.551353996205, 
68.5028425810333, 128.605282540213, 128.855928380748, 0)), class = c("bayesfactor_models", 
...
> **NOTE:** In order to correctly and precisely estimate Bayes Factors, you always need the 4 P's: **P**roper **P**riors ([1](https://doi.org/10.1016/j.jmp.2015.08.002), [2](https://www.tandfonline.com/doi/abs/10.1080/01621459.1995.10476572), [3](https://doi.org/10.1016/S0304-4076(00)00076-2)), and a **P**lentiful **P**osterior ([4](https://doi.org/10.1007/s11336-018-9648-3)).
...
```{r inclusion_brms, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r inclusion_brms2, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r JASP_all, message=FALSE, warning=FALSE, eval=FALSE}
library(BayesFactor)
...
```{r JASP_matched, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r JASP_Nuisance, message=FALSE, warning=FALSE, eval=FALSE}
