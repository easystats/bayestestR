...
***Special care should be taken when working with factors which have 3 or more levels.*** This section introduces the biased priors obtained when using the common *effects* factor coding (`contr.sum`), and the solution of using orthogonal factor coding (`contr.bayesian`) [as outlined in @rouder2012default, section 7.2].
...
The solution is to use *orthogonal* factor coding, a-la `contr.bayesian`, which can either specify this factor coding per-factor:

```{r}
contrasts(iris$Species) <- contr.bayesian
```

Or you can set it globally:

```{r, eval=FALSE}
options(contrasts = c('contr.bayesian', 'contr.poly'))
```
...
3. Only with `contr.bayesian`, the prior distributions of the differences is unbiased.
...
3. Only with `contr.bayesian`, the comparison of 3 (or more) levels the proper prior probability of - here that of 1/6 = 0.166.
