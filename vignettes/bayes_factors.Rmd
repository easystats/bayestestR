...
$$
Importantly, **Bayes factors come in many different flavours and cover a wide range applications**. For example, in the Bayesian framework, a Bayes factor can also be seen as the quantity by which some prior beliefs about the relative odds of two models are updated in light of the observed data - and in fact one way to compute Bayes factors is to divide posterior odds by prior odds, as we will soon demonstrate.

<!-- I would rephase the sentence that comes after "For example, ..." to smoothen the transition and be more straightforward. For example, the "can also be seen" is unclear, can *also* be seen besides what? -->

Critically, Bayes factors could be regrouped by two different applications: **testing parameters (coefficients) within models** and **comparing statistical models themselves**.

## Testing Models' Parameters with Bayes Factors

A ***Bayes factor for a single parameter*** can be used to answer the question:

> **Given the observed data, is the null hypothesis of an absence effect (parameter = 0) more, or less likely?**
