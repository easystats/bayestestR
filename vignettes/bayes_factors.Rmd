...
### Restricting More than 2 Factor-Levels

***Special care should be taken when restricting the order of more than 2 levels of the same factor.***

If we want to test an order restriction among 3 groups A, B, and C, the*a priori* probability of obtaining the order A > C > B is 1/6 (reach back to intro to stats year 1...). And so when testing and order restriction on the ordering of levels in a factor, we would the priors or be 1/6.... But...

For our example, we will be interested in the following order restrictions in the `iris` data (each line is a separate restriction):
...
```{r, eval=FALSE}
fit <- stan_glm(Sepal.Length ~ Species, data = iris,
                family = gaussian())
```
```{r, echo=FALSE, message=FALSE, warning=FALSE}
junk <- capture.output(fit <- stan_glm(Sepal.Length ~ Species, data = iris,
                                       family = gaussian()))
```
```{r}
em_ <- emmeans(fit, ~Species)
em_ # the posterior marginal means

bayesfactor_restricted(em_, fit, hypothesis = hyp)
```
...
You can read more about why this happens in [@rouder2012default, section 7.2], but for now, let's focus on the solution...

What we need to do, is use a different type of factor coding, available from the [`bfrms`](https://github.com/bayesstuff/bfrms) package as `contr.bayes`.

You can either specify this factor coding per-factor:
```{r eval=FALSE}
contrasts(iris$Species) <- bfrms::contr.bayes
```

Or you can set this option globally:
```{r, eval=FALSE}
contr.bayes <- bfrms::contr.bayes
options(contrasts = c('contr.bayes', 'contr.poly'))
```

Once this is set, simple run your analyses as usual:
```{r, eval=FALSE}
fit <- stan_glm(Sepal.Length ~ Species, data = iris,
                family = gaussian())
...
#>  setosa       5.01      4.86      5.16
#>  versicolor   5.94      5.78      6.07
#>  virginica    6.59      6.45      6.73
#> 
...
#>                               setosa > versicolor     0.51            0      0.00e+00
#>                                setosa > virginica     0.50            0      0.00e+00
#>                            versicolor > virginica     0.50            0      0.00e+00
#>  setosa    < virginica  & virginica  < versicolor     0.16            0      0.00e+00
#>  virginica < setosa     & setosa     < versicolor     0.17            0      0.00e+00
