...
$$
BF_{12}=\frac{P(D|M_1)}{P(D|M_2)}=\frac{Posterior~Odds}{Prior~Odds}
$$

Thus, Bayes factors can be defined in two ways:

- as a ratio quantifying **the relative probability of observed data** by two
models (since they can be computed by comparing the marginal likelihoods of the two
models),
...
distribution with hyperparameters of $mean = 0$ and $SD = 3$), using
`rstanarm` package:
...
models will be compared (in this case, the intercept-only model, which can be
considered as the *null* model):
...
that contain the main effect from which the interaction predictor is comprised
