...
This BF indicates the **likelihood of an effect of 0 (null effect) is `r round(test_group2$BFsd[1],2)` less likely under posterior**. In other words, a null effect is has become 1/`r round(test_group2$BFsd[1],2)` = `r 1/round(test_group2$BFsd[1],2)` time more likely given the observed data. Although the centre of distribution has shifted, it is still quite dense around the null.
...
#   geom_density(data = data.frame(posterior = posterior[posterior > 0]),
#                aes(x = posterior, color = "Posterior")) +
#   theme_classic()
...
> **Is the observed data more likely under Model 1, or Model 2?**
