...
The adoption of the Bayesian framework for applied statistics, especially in the social or psychological sciences, seems to be developing in two distinct directions, with one of the key topics marking their separation is their opinion about the **Bayes factor**. In short, some authors (e.g., the "Amsterdam school", led by [Wagenmakers](https://www.bayesianspectacles.org/)) advocate its use and emphasise its qualities as a statistical index, while others point to its limits and prefer instead the precise description of posterior distributions (using CIs, ROPEs, etc.). 
...
Alternativly, we could also pass our model directly as-is to `bayesfactor_savagedickey` to simultaniously test all of the model's parameters:
