...
This is done by comparing the density (likelihood) of the null value under the prior between the prior and posterior distributions, and is an approximation of a Bayes factor against the (point) null model:
...
This BF indicates the **likelihood of an effect of 0 (the point-null effect model) is `r round(test_group2$BF[1],2)` less likely given the data**. In other words, a null effect model has become 1/`r round(test_group2$BF[1],2)` = `r 1/round(test_group2$BF[1],2)` time more likely given the observed data. Thus, although the centre of distribution has shifted, it is still quite dense around the null.
