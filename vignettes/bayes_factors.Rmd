...
bayesfactor_savagedickey(posterior = posterior, prior = prior)
```

This BF indicates the **likelihood of an effect of 0 (null effect) is 0.76 less likely under posterior**. In other words, the observed data has `1/0.76 = 1.32` more chances to happen under the null hypothesis. Altough the center of distribution has shifted, it is still quite dense around the null.
...
bayesfactor_savagedickey(posterior = posterior, prior = prior, direction = ">")
```

As we can see, given that we have an *a priori* assumption about the direction of the effect (*that the effect is positive*), the presence of an effect is `1.5` times more likely than the null hypothesis.  indicating that given the observed data, the posterior mass has shifted away from the null value, giving some evidence against the null (note that a Bayes factor of `1.5` is still considered quite week evidence).
...
structure(list(Model = c("Petal.Length", "Species", "Species + Petal.Length", 
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
# dput(update(comparison, reference = 3))
