...
We can see that a-priori, this specific ordering of the 3 means has a proability of $\frac{1}{6}$ (1 of 6 possible orderings of 3 values), but after observing the data, this ordering is about ~4 times more likely than any other ordering.
...
If we zoomed-in on the null value $\theta_0$ - what does it mean for the null's credability to have become _lower_ in the posterior distribution?
...
if we have a prior hypotheses about the direction of the effect. This is similiar to the _dividing_ Bayes factor discussed above, but we are still comparing the (directional) alternative to the null (not between two directional hypotheses).
...
These are implamneted in `contr.equalprior()` and its siblings.
...
We can see that while all models have the very similiar posterior distributions,
