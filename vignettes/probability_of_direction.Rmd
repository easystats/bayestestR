...
- It is strongly correlated with the frequentist *p*-value, and can be used to draw some parallels and give some reference to readers non-familiar with Bayesian statistics.

However, this index does not give information about the magnitude and importance (the real "significance") of an effect, which is better achieved through other indices such as the [ROPE percentage](https://easystats.github.io/bayestestR/index.html#rope). In fact, indices of significance and existence are totally independent. You can have an effect with a *pd* of `99.99\%`, for which the whole posterior distribution is concentrated in a `[0.0001, 0.002]` range. In this case, **the effect is positive** with a high certainty, but also **not significant** (*i.e.*, very small).
...
The second approach relies on [**density estimation**](https://easystats.github.io/bayestestR/reference/estimate_density.html). It starts by estimating the density function (for which many methods are available), and then computing the [**area under the curve**](https://easystats.github.io/bayestestR/reference/area_under_curve.html) (AUC) of the density on the other side of 0. The density-based method could be considered as more precise (as the "simple" method's precision is directly related to the number of posterior draws), but strongly depends on the method used to estimate the density function.
