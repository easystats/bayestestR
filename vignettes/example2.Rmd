...
Now, let us run our `describe_posterior()` functio on that:
...
knitr::include_graphics("https://i1.wp.com/www.bayesianspectacles.org/wp-content/uploads/2018/07/LetsPokeAPizza.jpg")
```
...
***"I know that I know nothing, and especially not if *versicolor* and *virginica* differ in terms of Petal.Length"*, famously said Socrates**. Time to finally answer this answer this question!

### Versicolor *vs.* virginica

The same can be done with *t*-tests. Let's say we are particularly interested in two levels of the `Species` factor, *versicolor* and *virginica*. We will start by filtering out from `iris` the non-relevant observations corresponding to the *setosa* specie, and we will then visualise the observations and the distribution of the `Petal.Length` variable. 
