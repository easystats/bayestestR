...
Remember how we said that a correlation test actually compared two hypotheses, a null (absence of effect) with an altnernative one (presence of an effect)\? Well the [**Bayes factor (BF)**](https://easystats.github.io/bayestestR/articles/bayes_factors.html) allows us to do *exactly* that, by examining **under which of two models the observed data are more probable**: a model with the effect of interest, and a null model without the effect of interest. We can use `bayesfactor()` to compute the Bayes factor comparing those models (*and many more*):
...
**Bayes factors (BF)** are continuous measures of relative evidence, with a Bayes factor greater than 1 giving evidence in favor of one of the models (often reffered to as *the numerator*), and a Bayes factor smaller than 1 giving evidence in favour of the other model (*the denominator*). 
