...
It makes you fat by around `50.6` (the median). However, the uncertainty is quite high: **there is 90\% chance that the difference between the two feed types is between `9.15` and `91.34`.**

### ROPE Percentage

Testing wether this distribution is different from 0 doesn't make sense, as 0 is a single value. However, one way to assess **significance** could be to define an area around 0, which will consider as *equivalent* to zero (to no, - or negligible, effect). This is called the [**Region of Practical Equivalence (ROPE)**](https://easystats.github.io/bayestestR/articles/region_of_practical_equivalence.html), and is one way of testing the significance of parameters.
...
**- I am Prof Howard. An expert in chicks... I mean chickens. Just calling to let you know that based on my expert knownledge, an effect between -20 and 20 is negligible. Bye.**

---

Well, that's convenient. Now we know that we can define the ROPE as the `[-20, 20]` range. All effects within this range are considered as null. We can now compute the **proportion of the 90\% most probable values (the 90\% CI) which are not null**, *i.e.*, which are outside this range. 
...
**5.75\% of the 90\% CI can be considered as null**. Is that a lot? Based on our [**guidelines**](https://easystats.github.io/bayestestR/articles/guidelines.html), yes, it is a bit too much. **Based on this particular definition of ROPE**, we conclude that this effect is not significant (the probability of being negligible is too high).
...
**Yes.** One of the practice is for instance to use the **tenth** (`1/10 = 0.1`) of the standard deviation (SD) of the response variable.
...
Let's redefine our ROPE as the region within the `[-6.17, 6.17]` range. However, note that this can be directly obtained by the `rope_range` function :)
...
Well, with this reasonable definition of ROPE, we observe that that the 90\% of the posterior distribution of the effect does **not** overlap with the ROPE. Thus, we can conclude that **the effect is significant**.


### Probability of Direction (pd)

Maybe we are not interested in whether the effect is not-negligible. Maybe **we just want to know if this effect is positive or negative**. In this case, we can simply compute the proportion of the posterior that is positive, no matter the "size". 
...
We can conclude that **the effect is positive with a probability of 97.90\%**. We call this index the [**Probability of Direction (pd)**](https://easystats.github.io/bayestestR/articles/probability_of_direction.html). It can, in fact, be computed more easily with the following:

```{r message=FALSE, warning=FALSE, eval=TRUE}
p_direction(posteriors$feedsunflower)
```

Interestingly, it so happens that **this index is (in *most* of the cases) highly correlated to the frequentist *p*-value**. We could almost roughly infer the corresponding *p*-value with a simple transformation:
...
The frequentist model tells us that the difference is **positive and significant** (beta = 52.01, p = 0.04). 
