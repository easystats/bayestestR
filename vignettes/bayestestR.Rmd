...
- To characterize our effects, **no need of *p* values** or other cryptic indices. We simply describe the posterior distribution of the effect. For example, we can report the median, the 90% *Credible* Interval or [other indices](https://easystats.github.io/bayestestR/articles/4_Guidelines.html).


```{r echo=FALSE, fig.cap="Accurate depiction of a Bayesian user estimating a credible interval.", fig.align='center', out.width="50%"}
knitr::include_graphics("https://github.com/easystats/bayestestR/tree/master/man/figures/bayesianMaster.png")
```


*Note: Altough the very reason of this package is to advocate for the use of Bayesian statistics, please note that there are arguments supporting frequentist indices (see for instance [this thread](https://discourse.datamethods.org/t/language-for-communicating-frequentist-results-about-treatment-effects/934/16)). As always, the world is not black and white (p \< .001).*
...
Let's fit a simple frequentist linear regression between two numeric variables, `Sepal.Length` and `Petal.Length` from the famous [`iris`](https://en.wikipedia.org/wiki/Iris_flower_data_set) dataset, included in R.
...
*Fitting and interpreting frequentist models is so easy it is normal that people use it instead of the Bayesian framework... right?*
...
**That's it!** You fitted a Bayesian version of the model by simply using [`stan_glm()`](https://mc-stan.org/rstanarm/reference/stan_glm.html) instead of `lm()`. The conclusion that we can drawn, in this, are very similar. The effect (*the median of the effect's posterior distribution*) is about `0.41`, and it can be also be considered as *significant* in the Bayesian sense.

**So, ready to learn more?** Check out the [**next tutorial**](https://easystats.github.io/bayestestR/articles/1_Example1_GLM.html)!
